...
CREATE INDEX `fk_TiendaBienestarUN_Persona1_idx` ON `mydb`.`TiendaBienestarUN` (`directorID` ASC) VISIBLE;

CREATE INDEX `fk_TiendaBienestarUN_Persona2_idx` ON `mydb`.`TiendaBienestarUN` (`coordID` ASC) VISIBLE;

CREATE INDEX `fk_TiendaBienestarUN_ConBieneUniv1_idx` ON `mydb`.`TiendaBienestarUN` (`conBieneUnivID` ASC) VISIBLE;
...
CREATE INDEX `fk_ConvocGAI (GrArtInst)_has_Estudiante_Estudiante2_idx` ON `mydb`.`gaiTieneActividad` (`estID` ASC) VISIBLE;

CREATE INDEX `fk_ConvocGAI (GrArtInst)_has_Estudiante_ConvocGAI (GrArtIns_idx` ON `mydb`.`gaiTieneActividad` (`convGaiID` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `mydb`.`ConvocatoriaSeleccion` (
  ...
  `convDeporte` VARCHAR(50) NOT NULL,
